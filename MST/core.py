...
def is_cycling(graph:dict):
    ...
    def dfs(node, father_node=None):
        for neighbor_node in graph.get(node).keys():
            ...
            if not father_node:
                return False
            elif neighbor_node == father_node:
                continue
...
def generate_mst_kruskal(graph):
    ...
    for edge in edge_list:
        ...
        print(edge)
        print(is_cycling(graph_mst))
        if is_cycling(graph_mst):
            remove_edge(graph_mst, edge)
        # print(is_cycling(graph_mst), i, edge_number(graph_mst), node_number(graph_mst))
        # nodes = [int(i) for i in graph_mst.keys()]
        # nodes.sort()
        # print(nodes)
...
if __name__ == '__main__':
    ...
    graph = load_csv('test_graph1.csv')
    print_graph(graph, info=False)
    import time
    ...
    graph_mst1 = generate_mst_kruskal(graph)
    print('**********************kruskal************')
    print_graph(graph_mst1, info=True)
    print_graph(graph_mst1, info=False)
    time2 = time.time()
    graph_mst2 = generate_mst_prime(graph)
    time3 = time.time()
    print('**********************prime************')
    print_graph(graph_mst2)
    print_graph(graph_mst2, info=False)
        # edge_number_list.append(i * 100)
        # kruskal_time_list.append(time2 - time1)
        # prime_time_list.append(time3 - time2)
